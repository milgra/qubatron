...
zombie_parts_t zombie_parts = {
    ...
    .lshoul = (v4_t){33.0, 170.0, 26.0, 14.0},
    .lhand  = (v4_t){-10.0, 70.0, 26.0, 3.0},

    .rshoul = (v4_t){75.0, 170.0, 26.0, 14.0},
    .rhand  = (v4_t){120.0, 70.0, 26.0, 3.0},

    .lleg  = (v4_t){38.0, 120.0, 26.0, 26.0},
    .lfoot = (v4_t){8.0, -10.0, 26.0, 8.0},

    .rleg  = (v4_t){70.0, 120.0, 26.0, 26.0},
    .rfoot = (v4_t){100.0, -10.0, 26.0, 8.0}
...
zombie_t zombie_init()
{
    ...
    res.newparts.lshoul = v4_add(res.newparts.hip, (v4_t){-47.0, 50.0, 0.0, 0.0});
    res.newparts.lhand  = v4_add(res.newparts.hip, (v4_t){-62.0, -50.0, 0.0, 0.0});
    res.newparts.rshoul = v4_add(res.newparts.hip, (v4_t){53.0, 50.0, 0.0, 0.0});
...
void zombie_update(zombie_t* zombie, float lighta, float dir, v4_t pos)
{
    ...
    v4_t lshoulv = v4_xyzw(quat_rotate(back_rot, (v3_t){-47.0, 50.0, 0.0}));
    v4_t lhndv   = v4_xyzw(quat_rotate(back_rot, (v3_t){-62.0 + sin(lighta) * 15.0, -50.0, 30.0}));
    v4_t rshoulv = v4_xyzw(quat_rotate(back_rot, (v3_t){53.0, 50.0, 0.0}));
    ...
    rshoulv = v4_sub(zombie_parts.rshoul, zombie_parts.hip);
    rshoulv = v4_xyzw(quat_rotate(back_rot, v4_xyz(rshoulv)));

    rshoulv.w = 0.0;
    /* v4_t rhndv = v4_sub(zombie_parts.rhand, zombie_parts.hip); */
    /* rhndv.w    = 0.0; */

    lshoulv = v4_sub(zombie_parts.lshoul, zombie_parts.hip);
    lshoulv = v4_xyzw(quat_rotate(back_rot, v4_xyz(lshoulv)));
    ...
    zombie->newparts.lshoul = v4_add(zombie->newparts.hip, lshoulv);
    zombie->newparts.lhand  = v4_add(zombie->newparts.hip, lhndv);
    zombie->newparts.rshoul = v4_add(zombie->newparts.hip, rshoulv);
	...
	.arr = {
	    ...
	    zombie->newparts.neck,
	    zombie->newparts.hip,
	    zombie->newparts.rshoul,
	    zombie->newparts.rhand,
	    zombie->newparts.lshoul,
